...
Landed so far:
- **Player side**: `AnthiasAPIToken` bearer-token auth (`src/anthias_server/lib/auth.py`, `src/anthias_server/api/models.py`) plus REST management of it at `/api/v2/auth/tokens`; the `fleet_link` app (`PlayerIdentity`, `FleetPairing`, `SyncState`, `Command` models, plus `fleet_link/tasks.py`'s `poll_fleet_pairing` Celery-beat task) and `Asset.origin`/`authority`/`fleet_media_id`/`source_deployment_id`/`deployed_version`; read-only `/api/v2/player/identity`, `/api/v2/player/policy`, `/api/v2/playlists`, `/api/v2/schedules`, `/api/v2/management/commands`, `/api/v2/management/sync-state`; and the pairing handshake itself — `/api/v2/player/pairing/{start,status,cancel}` plus the outbound poll/ack calls `poll_fleet_pairing` makes against a Fleet Server. A completed pairing (`FleetPairing.status='active'`) deliberately leaves all four per-domain authority fields at their `local` default — negotiating them is the desired-state sync engine's job (next), not pairing's.
- **Fleet Server side**: the service itself (`src/anthias_fleet_server/`) — `core` app (`Organization`, `Membership`/role, `Group`, `Player`, `Media`, `Playlist`/`PlaylistItem`, `Deployment`, `PairingRequest`, `PlayerCredential` models + Django admin) and `api` app (DRF `ModelViewSet`s under `/api/`, RBAC via `api/permissions.py`'s `HasMinimumRole`, org-scoped so a client only ever sees its own org's rows, plus the unauthenticated `/api/pairing/poll` and device-credential-authenticated `/api/pairing/ack` endpoints in `api/pairing_views.py` — see `PairingRequest`'s docstring for the "reissue credential until acknowledged" handshake). Media upload is content-addressed (SHA-256, re-uploading identical bytes returns the existing row rather than duplicating storage). `bootstrap_org` management command provisions the first Organization + Owner. Dev stack: `docker-compose.fleet.dev.yml` (`fleet-server` + Postgres `fleet-db`, port 8100) — separate from the Player's own `docker-compose.dev.yml`, run either or both together.
- **Pairing protocol (Phase 4) is landed**: a real Player can pair with a real Fleet Server — the Player generates the pairing code pair locally, polls `POST {fleet_base_url}/api/pairing/poll` via Celery beat, a Fleet Operator+ approves via `POST /api/pairing-requests/<id>/approve/` (creating or matching a `Player` row by `device_id`), and the Player's next poll receives and acknowledges a `PlayerCredential`. Credentials can be revoked fleet-side (`POST /api/players/<id>/revoke-credential/`).
- **Not yet built**: the desired-state sync engine (nothing reconciles `Asset` rows against Fleet-side `Deployment`s yet, so pairing has no visible effect on content today), and command delivery/execution (`Command` rows are never created by anything). A paired Player's `device_credential` is never used for anything past the initial `/api/pairing/ack` call until the sync engine lands.
